...
## 13.1 定期更新との組み合わせ（NDJSON）

```bash
disk-smi --json -l 5
```

`--json`と`-l`/`--loop`を組み合わせた場合、更新間隔ごとに本章のJSONドキュメントを1件、改行区切りで1行ずつ出力する（[NDJSON](https://github.com/ndjson/ndjson-spec)）。各行は独立した1つのJSONドキュメントであり、`generated_at`にその時点のタイムスタンプを持つ。画面クリアやANSIエスケープは一切出力しないため、TTY・パイプ・ファイルへのリダイレクトのいずれでも同じ形式になる。

```bash
disk-smi --json -l 5 >> smart-log.ndjson
```

`--json-pretty`は`-l`/`--loop`と組み合わせられない。整形済みJSONは1件が複数行になり、NDJSONとしての「1行1レコード」の前提が崩れるためである。
...
## 17.1 `--check`モードの終了コード

`--check`は、cron・launchd・Nagios系監視ツールからの定期実行を想定した専用モードである。検出された（または指定した）ドライブごとに1行のステータス行を出力し、健康状態に応じて次の終了コードを返す。

| コード | 内容                              |
| --: | ------------------------------- |
|   0 | すべてのドライブが`GOOD`                  |
|   1 | いずれかのドライブが`CAUTION`または`UNKNOWN`   |
|   8 | いずれかのドライブが`CRITICAL`             |

慣習的なNagios形式は`0`/`1`/`2`/`3`だが、`2`は本仕様の一般終了コード表で既に「CLI引数エラー」として使われている。同じ終了コードに2つの意味を持たせると監視スクリプト側で誤判定を招くため、空いている`1`を警告用に、`8`（一般終了コード表が使っていない最小の番号）を危険用に割り当てる。`UNKNOWN`（データ取得不能）は、「取得できないことを正常と解釈しない」という4章の原則に従い、警告（終了コード1）側に含める。

ドライブ読み取り自体に失敗した場合（対象SSDが見つからない、SMART取得失敗、権限不足など）は、通常表示と同じ一般終了コード（2, 3, 4, 5, 6, 7）を使う。

`--check`は`--json`、`--json-pretty`、`--summary`、`-l`/`--loop`と同時に指定できない。
